...
namespace DirectGraphicalModels 
{
	...
	const std::vector<int>& CRTrees::getActiveVars()
	{
	...
	void CRTrees::startTraining(const Ptr<ml::TrainData>& trainData, int flags)
	{
		CDTrees::startTraining(trainData, flags);
	...
	bool CRTrees::train(const Ptr<ml::TrainData>& trainData, int flags)
	{
		...
		if (w->data->getLayout() == ml::COL_SAMPLE)
	...
	void CRTrees::writeTrainingParams(FileStorage& fs) const
	{
		CDTrees::writeTrainingParams(fs);
		fs << "nactive_vars" << rparams.nactiveVars;
	}

	void CRTrees::write(FileStorage& fs) const
	{
		if (roots.empty())
	...
	void CRTrees::readParams(const FileNode& fn)
	{
		CDTrees::readParams(fn);

		FileNode tparams_node = fn["training_params"];
		rparams.nactiveVars = (int)tparams_node["nactive_vars"];
	}

	void CRTrees::read(const FileNode& fn)
	{
		clear();
