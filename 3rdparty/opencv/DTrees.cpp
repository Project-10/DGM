...
namespace DirectGraphicalModels 
{
	...
	void CDTrees::startTraining(const Ptr<ml::TrainData>& data, int)
	{
		clear();
		...
		_isClassifier = data->getResponseType() == ml::VAR_CATEGORICAL;
	...
	void CDTrees::endTraining()
	{
		w.release();
	}

	bool CDTrees::train(const Ptr<ml::TrainData>& trainData, int flags)
	{
	...
	const vector<int>& CDTrees::getActiveVars()
	{
		return varIdx;
	}

	int CDTrees::addTree(const vector<int>& sidx)
	{
	...
	void CDTrees::setDParams(const TreeParams& _params)
	{
		params = _params;
	}

	int CDTrees::addNodeAndTrySplit(int parent, const vector<int>& sidx)
	{
	...
	int CDTrees::findBestSplit(const vector<int>& _sidx)
	{
		...
		for (vi_ = 0; vi_ < nv; vi_++)
		{
			...
			if (varType[vi] == ml::VAR_CATEGORICAL)
			{
				if (_isClassifier)
	...
	void CDTrees::calcValue(int nidx, const vector<int>& _sidx)
	{
	...
	CDTrees::WSplit CDTrees::findSplitOrdClass(int vi, const vector<int>& _sidx, double initQuality)
	{
	...
	void CDTrees::clusterCategories(const double* vectors, int n, int m, double* csums, int k, int* labels)
	{
	...
	CDTrees::WSplit CDTrees::findSplitCatClass(int vi, const vector<int>& _sidx, double initQuality, int* subset)
	{
	...
	CDTrees::WSplit CDTrees::findSplitOrdReg(int vi, const vector<int>& _sidx, double initQuality)
	{
	...
	CDTrees::WSplit CDTrees::findSplitCatReg(int vi, const vector<int>& _sidx, double initQuality, int* subset)
	{
		const double* weights = &w->sample_weights[0];
		const double* responses = &w->ord_responses[0];
		int n = (int)_sidx.size();
	...
	int CDTrees::calcDir(int splitidx, const vector<int>& _sidx, vector<int>& _sleft, vector<int>& _sright)
	{
	...
	double CDTrees::updateTreeRNC(int root, double T, int fold)
	{
	...
	bool CDTrees::cutTree(int root, double T, int fold, double min_alpha)
	{
	...
	float CDTrees::predictTrees(const Range& range, const Mat& sample, int flags) const
	{
		...
		double sum = 0.;
		int lastClassIdx = -1;
		const float MISSED_VAL = ml::TrainData::missingValue();
	...
	float CDTrees::predict(InputArray _samples, OutputArray _results, int flags) const
	{
	...
	void CDTrees::writeTrainingParams(FileStorage& fs) const
	{
	...
	void CDTrees::writeParams(FileStorage& fs) const
	{
	...
	void CDTrees::writeSplit(FileStorage& fs, int splitidx) const
	{
		...
		if (varType[vi] == ml::VAR_CATEGORICAL) // split on a categorical var
		{
	...
	void CDTrees::writeNode(FileStorage& fs, int nidx, int depth) const
	{
		const Node& node = nodes[nidx];
		fs << "{";
		fs << "depth" << depth;
	...
	void CDTrees::writeTree(FileStorage& fs, int root) const
	{
		fs << "nodes" << "[";
	...
	void CDTrees::write(FileStorage& fs) const
	{
		writeParams(fs);
		writeTree(fs, roots[0]);
	}

	void CDTrees::readParams(const FileNode& fn)
	{
		...
		if (isLegacy && (int)varType.size() <= varAll)
		{
			...
			extendedTypes[varAll] = (uchar)(_isClassifier ? ml::VAR_CATEGORICAL : ml::VAR_ORDERED);
		...
		if (isLegacy)
		{
			...
			for (; i < size; ++i)
			{
				...
				if (varType[i] == ml::VAR_CATEGORICAL) // only categorical vars are represented in catMap
				{
	...
	int CDTrees::readSplit(const FileNode& fn)
	{
		Split split;
		...
		if (varType[vi] == ml::VAR_CATEGORICAL) // split on categorical var
		{
	...
	int CDTrees::readNode(const FileNode& fn)
	{
		Node node;
	...
	int CDTrees::readTree(const FileNode& fn)
	{
	...
	void CDTrees::read(const FileNode& fn)
	{
		clear();
		readParams(fn);
