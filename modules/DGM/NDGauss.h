// N-Dimensional Gauss function class interface
...
*/
		CNDGauss(const Mat &mu);
/// @endcond
		DllExport CNDGauss(const CNDGauss &rhs);
		DllExport ~CNDGauss(void);

		DllExport CNDGauss & operator= (const CNDGauss & rhs);
		DllExport CNDGauss & operator+=	(const CNDGauss & rhs);
/**
...
*/
		DllExport const	CNDGauss operator+ (const CNDGauss & rhs) const	{return CNDGauss(*this) += rhs;}
...
*/
		...
		*/
		DllExport double		getKullbackLeiberDivergence(CNDGauss &x) const;
